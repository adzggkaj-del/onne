...
const DepositDialog = ({ open, onOpenChange }: DepositDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className={step === "detail" ? "max-w-lg max-h-[90vh] overflow-y-auto" : "max-w-sm"}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {step === "detail" && (
              <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={() => { setStep("method"); setConfirmed(false); }}>
